package ru.job4j.streram;

import org.junit.Test;

import java.util.List;
import java.util.Map;

import static org.junit.Assert.*;
import static ru.job4j.streram.CountinMethod.*;


public class CountinMethodTest {

    @Test
    public void groupAndCount() {
        Company c1 = new Company("Apple");
        Company c2 = new Company("Samsung");
        Company c3 = new Company("Microsoft");
        Company c4 = new Company("Amazon");
        Worker w1 = new Worker(20, c1);
        Worker w2 = new Worker(25, c1);
        Worker w3 = new Worker(30, c2);
        Worker w4 = new Worker(35, c2);
        Worker w5 = new Worker(40, c2);
        Worker w6 = new Worker(45, c3);
        Worker w7 = new Worker(50, c3);
        Worker w8 = new Worker(30, c3);
        Worker w9 = new Worker(32, c4);

        Map<String, Long> result = Map.of(
                "Apple", 2L,
                "Samsung", 3L,
                "Microsoft", 3L,
                "Amazon", 1L
        );
        assertEquals(result, CountinMethod.groupAndCount(List.of(w1, w2, w3, w4, w5, w6, w7, w8,w9)));
    }
}